...
interface AdditionalServiceProps {
    additionalServices: AddServiceI[];
    addService: number[];
    setAddService: (addServiceId: number[]) => void;
    setService: (serviceId: number) => void;
    service: number;
    setAddServicePass: (pass: boolean) => void;
}

export default function AdditionalService({ additionalServices, addService, setAddService, setService, service, setAddServicePass }: AdditionalServiceProps) {
    ...
    const addServiceRef = useRef<number[]>(addService);
    ...
    useEffect(() => {
        ...
        const updatedCheck = additionalServices.map(addServiceItem => {
            return addService.some(serviceCheck => serviceCheck === addServiceItem.id);
        });
        setAddServiceCheck(updatedCheck);

        const updatedDisabled = additionalServices.map(addServiceItem =>
            (Array.isArray(addServiceItem.match) && addServiceItem.match.includes(service)) ||
            addServiceItem.id === service ||
            (Array.isArray(addServiceItem.match) && addServiceItem.match.some(matchId => addServiceRef.current.some(serviceRef => serviceRef === matchId)))
        );
    ...
    const handleCheckboxChange = (index: number) => {
        ...
        if (updatedCheck[index]) {
            setAddService([...addService, additionalServices[index].id]);
            addServiceRef.current = [...addService, additionalServices[index].id];
            const updatedDisabled = additionalServices.map(addServiceItem =>
                (Array.isArray(addServiceItem.match) && addServiceItem.match.includes(service)) ||
                addServiceItem.id === service ||
                (Array.isArray(addServiceItem.match) && addServiceItem.match.some(matchId => addServiceRef.current.some(serviceRef => serviceRef === matchId)))
            ...
            setAddService(addService.filter(service => service !== additionalServices[index].id));
            addServiceRef.current = addService.filter(service => service !== additionalServices[index].id);
            const updatedDisabled = additionalServices.map(addServiceItem =>
                (Array.isArray(addServiceItem.match) && addServiceItem.match.includes(service)) ||
                addServiceItem.id === service ||
                (Array.isArray(addServiceItem.match) && addServiceItem.match.some(matchId => addServiceRef.current.some(serviceRef => serviceRef === matchId)))
    ...
    return (
        <div id="bookOnlineAddService" className="bookOnlineServicesContainer">
            <h2 id="bookOnlineAddServiceBack" className="bookOnlineServicesBack" onClick={() => {setAddService([]); setService(-1);}}>Back</h2>
