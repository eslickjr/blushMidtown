...
export default function UpdateCardModal({updateCardModalOpen, setUpdateCardModalOpen, cardToUpdate, setClient}: UpdateCardModalProps) {
    ...
    const updateCard = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const updatedCard: UpdateCard = {
            ...
            cardType: cardToUpdate.cardType,
            expiryDate: expiryDate,
            zipCode: zipCode,
            isPrimary: isPrimary
        };
        console.log("isPrimary:", e.currentTarget.updateCardMakePrimaryCheckbox.checked);
        console.log("Card to update:", cardToUpdate.isPrimary);

        if (isPrimary !== cardToUpdate.isPrimary) {
            setClient((prevClient: any) => ({
                ...prevClient,
                cards: prevClient.cards.map((card: UpdateCard) =>
                    card.cardNumber === cardToUpdate.cardNumber
                        ? { ...updatedCard }
            ...
            setClient((prevClient: any) => ({
                ...prevClient,
                cards: prevClient.cards.map((card: UpdateCard) =>
                    card.cardNumber === cardToUpdate.cardNumber ? updatedCard : card
                )
            }));
        }
                    ...
                    <div id="updateCardFormInputContainer">
                        ...
                        <div id="updateCardMakePrimaryContainer">
                            <div id="updateCardMakePrimaryCheckboxContainer">
                                <input type="checkbox" id="updateCardMakePrimaryCheckbox" checked={cardChecked} name="updateCardMakePrimaryCheckbox" onClick={() => setCardChecked(!cardChecked)}/>
